...
from sklearn import linear_model,preprocessing,ensemble,neighbors,tree,svm
...
class TrainingData(object):
    ...
    def readfile(self):
        ...
        for i, row in enumerate(user_r):
            if i > 0 and sh.shop_mall[row[1]]=='m_822':
                ...
                wifi=wifi.replace('|',"',")
                wifi="[['"+wifi+"]"
                wifi_list0=eval(wifi)
                wifi_list=[]
                temp_set=set([])  # 避免一条记录重复出现同一WiFi
                for val in wifi_list0:
                    if val[0] not in temp_set:
                        wifi_list.append(val)
                        temp_set.add(val[0])

                row[5]=wifi_list  # row=[shop_id,time_stamp,int_time,longitude,latitude,[[wifi1,value],...]]
    ...
    def shopWifiFreq(self):
        for key,val in self.shop_catg.items():  # 一个shop
            ...
            self.shop_top10_wifi[key] = [val2[1] for key2, val2 in enumerate(wifi_list) if (val2[0]+0.0)/len(val)>=0.1]  # 取top10 WiFi的id
    ...
    def getInputData(self):  # 获取模型需要的输入数据，经纬度、WiFi的值(该商场所有店铺tip10 Wifi的并集)，以及类别标签shop_id
        ...
        for key in self.shop_wifi:  # 剔除Wifi种类小于10的类别（店铺）(经统计发现等同于剔除样本数小于10的店铺）
            ...
            for val1 in self.shop_catg[key]:
                wifi_dict0={}  # {wifiname:[value,sequence],...}
                for j,val2 in enumerate(val1[5]):
                    wifi_dict0[val2[0]]=[val2[1],j]
                examp2 = [0] * (3 + len(self.mall_wifi[mall_id1])*2)  # 经纬度 + 该商场WiFi总数乘2（含WiFi出现顺序） + 加shop_id(label)
                ...
                for i, wifis in enumerate(self.mall_wifi[mall_id1]):  # 获取该顾客记录在该shop对应的10个WiFi值，没有信号取值-120
                    if wifis in wifi_dict0:
                        examp2[i*2 + 2:i*2+4] = wifi_dict0[wifis]
                        # examp2[i+2]=wifi_dict0[wifis][0]/(wifi_dict0[wifis][1]+1.0)
                    else:
                        examp2[i*2 + 2:i*2+4] = [-120,0]
                        # examp2[i + 2] = -114
...
def tuning():
    ...
    trd.shopWifiFreq()
    trd.getInputData()
    temp = trd.model_input_data['m_822']
    ...
    adb_clf = ensemble.RandomForestClassifier(n_estimators=300)
    # sm = svm.NuSVC()
    adb_clf.fit(X_train, y_train)
    # scores = cross_val_score(sm, x, y, cv=4)
    # print 'average accuracy of kfold: ',np.mean(scores)
