...
class TrainingData(object):
    ...
    def shopWifiFreq(self):
        for key,val in self.shop_catg.items():  # 一个shop
            ...
            self.shop_top10_wifi[key] = [val2[1] for key2, val2 in enumerate(wifi_list) if (val2[0]+0.0)/len(val)>=0.05]#key2 < 10]  # 取top10 WiFi的id
...
def main():
    ...
    f = open(r"C:\Users\Feng\Desktop\CustomerPosition\results\RandomForestResult_20171119_1.csv",
    ...
    def shopWifiFreq(self):
        for key,val in self.shop_catg.items():  # 一个shop
            ...
            for key1, val1 in wifi_dict.items():
                wifi_list.append([val1, key1])  # 把frequency值放在第一个，对应的WiFi名字放第二，以便下一步直接排序
            wifi_list.sort(reverse=True)  # order by wifi frequency in descending
            self.shop_top10_wifi[key] = [val2[1] for key2, val2 in enumerate(wifi_list) if (val2[0]+0.0)/len(val)>=0.1]  # 取店铺WiFi出现频率大于0.1的WiFi
...
def mallPrecision():
    ...
    for key,val in trd.model_input_data.items():
        ...
        mall_precs.append([key,len(x[0]),acy])

    f = open(r"C:\Users\Feng\Desktop\CustomerPosition\results\precision_20171118.csv",
             "wb")  # 'wb' instead of 'w' can remove blank rows
    writer = csv.writer(f)
    title = ['mall_id', 'feature_num','precision']
    writer.writerow(title)
