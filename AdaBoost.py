...
from sklearn import linear_model,preprocessing,ensemble,neighbors,tree
...
def accuracy(x,y):
    ...
    if xl==yl:
        ...
        print 'Error: compared lists do not have the same length!'
...
def main():
    ...
    for key,val in tsd.predict_examples.items():    # 对每一个mall训练一个模型以及预测值
        ...
        carttree = tree.DecisionTreeClassifier(max_depth=50,min_samples_split=10,min_samples_leaf=10)
        adb_clf = ensemble.AdaBoostClassifier(carttree, n_estimators=60,learning_rate=1)
    ...
    f = open(r"C:\Users\Feng\Desktop\CustomerPosition\results\AdaBoostTreeResult_20171117.csv",
...
class TrainingData3839(object):  # 只读取这一个mall_3839的数据做实验
    ...
    def readfile(self):
        ...
        for i, row in enumerate(user_r):
            if i > 0 and sh.shop_mall[row[1]]=='m_7800':  # 只取这一个mall的数据做实验
                del row[0]  # delete user_id, not need
                tm=int(row[1][-5:-3])*60+int(row[1][-2:])
                row.insert(2,tm)
    ...
    def shopWifiFreq(self):
        for key,val in self.shop_catg.items():  # 一个shop
            ...
            for key1, val1 in wifi_dict.items():
                wifi_list.append([val1, key1])  # 把frequency值放在第一个，对应的WiFi名字放第二，以便下一步直接排序
            wifi_list.sort(reverse=True)  # order by wifi frequency in descending
            self.shop_top10_wifi[key] = [val2[1] for key2, val2 in enumerate(wifi_list) if key2<10]#(val2[0]+0.0)/len(val)>=0.1]  # 取店铺WiFi出现频率大于0.1的WiFi
...
def tuning():
    ...
    trd.shopWifiFreq()
    trd.getInputData()
    temp = trd.model_input_data['m_7800']
    ...
    X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.2,random_state=42)
    # carttree = tree.DecisionTreeClassifier(max_depth=50, min_samples_split=10, min_samples_leaf=5,
                                           # class_weight='balanced')
    # adb_clf = ensemble.AdaBoostClassifier(carttree, n_estimators=80, learning_rate=1)
...
def mallPrecision():
    ...
    trd.getInputData()
    mall_precs=[]
    for key,val in trd.model_input_data.items():
        x = [val1[:-1] for val1 in val]
        y = [val2[-1] for val2 in val]

        X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=42)
        carttree = tree.DecisionTreeClassifier( max_depth=50, min_samples_split=10,
                                               min_samples_leaf=10, class_weight='balanced')
        adb_clf = ensemble.AdaBoostClassifier(carttree, n_estimators=40, learning_rate=1.5)
        adb_clf.fit(X_train, y_train)

        prediction = adb_clf.predict(X_test)
        acy=accuracy_score(y_test, prediction)
        print key,len(x[0]),acy
        mall_precs.append([key,acy])

    f = open(r"C:\Users\Feng\Desktop\CustomerPosition\results\precision_20171116.csv",
             "wb")  # 'wb' instead of 'w' can remove blank rows
    writer = csv.writer(f)
    title = ['mall_id', 'precision']
    writer.writerow(title)
    for vl in mall_precs:
        writer.writerow(vl)
    f.close()
    t1 = time.time()
    print t1 - t0
...
def minalchange():
    result = {}
    f = open(r"C:\Users\Feng\Desktop\CustomerPosition\results\minalResult_20171116.csv", "rU")
    reader = csv.reader(f)
    for rw in reader:
        result[rw[0]]=rw[1]
    f.close()
    ...
    key='m_1375'
    val =tsd.predict_examples[key]    # 对每一个mall训练一个模型以及预测值
    x_train = [valx[:-1] for valx in trd.model_input_data[key]]
    y_train = [valy[-1] for valy in trd.model_input_data[key]]  # training data
    row_id = []
    x_test = []
    ...
    # x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=42)
    carttree = tree.DecisionTreeClassifier(max_depth=50, min_samples_split=10, min_samples_leaf=5,class_weight='balanced')
    adb_clf = ensemble.AdaBoostClassifier(carttree, n_estimators=40, learning_rate=1.6)
    adb_clf.fit(x_train, y_train)

    prediction = adb_clf.predict(x_test)

    for j in range(len(prediction)):
        result[row_id[j]]=prediction[j]

    fi=open(r"C:\Users\Feng\Desktop\CustomerPosition\results\minalResult_20171116.csv", "wb")
